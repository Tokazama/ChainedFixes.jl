...
const StartsWith{T} = Base.Fix2{typeof(startswith),T}
const EndsWith{T} = Base.Fix2{typeof(endswith),T}
const Not{T} = (typeof(!(sum)).name.wrapper){T}
const In{T} = Base.Fix2{typeof(in),T}
const NotIn{T} = (typeof(!in(Any)).name.wrapper){Base.Fix2{typeof(in),T}}
const NotApprox{T,Kwargs} = (typeof(!in(Any)).name.wrapper){Approx{T,Kwargs}}
const Less{T} = Union{Base.Fix2{typeof(<),T},Base.Fix2{typeof(isless),T}}
const Equal{T} = Union{Base.Fix2{typeof(==),T},Base.Fix2{typeof(isequal),T}}
const NotEqual{T} = Base.Fix2{typeof(!=),T}
const Greater{T} = Base.Fix2{typeof(>),T}
const GreaterThanOrEqual{T} = Base.Fix2{typeof(>=),T}
const LessThanOrEqual{T} = Base.Fix2{typeof(<=),T}
...
function closest(collection, x)
    if eltype(collection) <: Number
        ...
        throw(MethodError("closest not defined for collection with eltype $(eltype(collection))"))
    end
end
...
const Closest{T} = Base.Fix2{typeof(closest),T}
...
Synonymous with bitwise `&` operator but may be used to chain multiple `Base.Fix1` or
`Base.Fix2` operations. The `⩓` (`\\And<TAB>`) operator may be used in its place (e.g., `x ⩓ y`).
...
```
"""
and(x, y::Function, z::Function) = y(x) ? z(x) : false
and(x, y::Bool, z::Function) = y ? z(x) : false
and(x, y::Function, z) = y(x) ? z : false
...
Synonymous with bitwise `|` operator but may be used to chain multiple `Base.Fix1` or
`Base.Fix2` operations. The `⩔` (`\\Or<TAB>`) operator may be used in its place (e.g., `x ⩔ y`).
...
```
"""
or(x, y::Function, z::Function) = y(x) ? true : z(x)
or(x, y::Bool, z::Function) = y ? true : z(x)
or(x, y::Function, z) = y(x) ? true : z
...
function print_fixed(io::IO, @nospecialize(f::NFix))
